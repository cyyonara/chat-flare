...
export const getChatPhotos = expressAsyncHandler(async (req: IRequest, res: Response) => {
  const { chatId } = req.params;
  const { page, limit } = req.query;
  const { parsedPage, parsedLimit } = parsePaginationData(
    page as string,
  ...
  const offset = (parsedPage - 1) * parsedLimit;
  const messagePhotos = await Message.find({ chatId, isImage: true })
    .skip(offset)
    .limit(parsedLimit)
    .populate({
      path: 'sender.user receivers.user reactors.user',
      select: '_id username email profilePicture',
      model: User,
    });

  const messagesCount = await Message.countDocuments({ chatId, isImage: true });
