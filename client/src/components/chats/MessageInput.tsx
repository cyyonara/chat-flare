...
export default function MessageInput({ isSuccess }: IProps) {
  ...
  const setCachedMessagesAndUpdate = (message: string | File, callback?: Function) => {
    const statusId = uuid();
    const _id = uuid();
          ...
          queryClient.setQueryData(
            ...
            (queryData: InfiniteData<IPaginatedChats>): InfiniteData<IPaginatedChats> => {
              const filteredPages = queryData.pages.map((page) => ({
                ...page,
                chats: page.chats.filter((chat) => chat._id !== data._id),
              }));
  ...
  return (
    ...
    >
      <div className='flex items-center gap-x-2'>
        <input type='file' hidden ref={imageInputRef} onChange={handleSendImage} />
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger
              type='button'
