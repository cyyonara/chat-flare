...
export default function Message({
  ...
  const displayProfile = (): boolean => {
    if (currentUserId !== sender._id) {
      const nextPage: IPaginatedFetchedMessages | undefined = pages[pageIndex + 1];
      const previousPage: IPaginatedFetchedMessages | undefined = pages[pageIndex - 1];
  ...
  return (
    <>
      <AnimatePresence>
        {showReactorsList && (
          <ReactorsList reactors={reactors} close={() => setShowReactorsList(false)} />
        )}
      </AnimatePresence>
      <div
          ...
          >
            <div className='relative'>
              ...
              {reactors.length > 0 && (
                <MessageReactions
                  reactors={reactors}
                  variant={currentUserId === sender._id ? 'secondary' : 'primary'}
                  position={currentUserId === sender._id ? 'left' : 'right'}
            ...
            <AnimatePresence>
              {showReactionPicker && (
                <ReactionPicker
                  ...
                  reactors={reactors}
                  closeReactionPicker={() => setShowReactionPicker(false)}
                />
              )}
            </AnimatePresence>
            <div
            ...
            >
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent className='text-xs'>Add Reaction</TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
          </div>
